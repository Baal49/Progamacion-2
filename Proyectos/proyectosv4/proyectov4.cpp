...
bool cargarTienda(Tienda* tienda){
    if(!tienda) return false;
    const char* ruta = "tienda.bin";
    fstream archivo(ruta, ios::in | ios::binary);
    if(!archivo){
        return false;
    }
    archivo.read(reinterpret_cast<char*>(&tienda->nombre), sizeof(tienda->nombre));
    archivo.read(reinterpret_cast<char*>(&tienda->rif), sizeof(tienda->rif));
    archivo.read(reinterpret_cast<char*>(&tienda->siguienteIdProducto), sizeof(tienda->siguienteIdProducto));
    archivo.read(reinterpret_cast<char*>(&tienda->siguienteIdProveedor), sizeof(tienda->siguienteIdProveedor));
    archivo.read(reinterpret_cast<char*>(&tienda->siguienteIdTransaccion), sizeof(tienda->siguienteIdTransaccion));
    archivo.close();
    return true;
}

bool guardarTienda(Tienda* tienda){
    if(!tienda) return false;
    const char* ruta = "tienda.bin";
    fstream archivo(ruta, ios::out | ios::binary | ios::trunc);
    if(!archivo){
        return false;
    }
    archivo.write(reinterpret_cast<const char*>(&tienda->nombre), sizeof(tienda->nombre));
    archivo.write(reinterpret_cast<const char*>(&tienda->rif), sizeof(tienda->rif));
    archivo.write(reinterpret_cast<const char*>(&tienda->siguienteIdProducto), sizeof(tienda->siguienteIdProducto));
    archivo.write(reinterpret_cast<const char*>(&tienda->siguienteIdProveedor), sizeof(tienda->siguienteIdProveedor));
    archivo.write(reinterpret_cast<const char*>(&tienda->siguienteIdTransaccion), sizeof(tienda->siguienteIdTransaccion));
    archivo.close();
    return true;
}
...
void inicializarTienda(Tienda* tienda){
    ...
    // Inicializa arreglos dinámicos para operaciones en memoria
    int cap = 5;
...
void Crearproveedor(Tienda* tienda){
    if(!tienda){ cout<<"Tienda no inicializada."<<endl; return; }
    const char* ruta = "proveedores.bin";
    fstream archivo(ruta, ios::out | ios::binary | ios::app);
    if(!archivo){ cout<<"No se puede abrir o crear el archivo de proveedores: "<<ruta<<"."<<endl; return; }
    ...
    for(int n=0;n<cantidad;n++){
        ...
        while(true){
            ...
            if(!getline(cin, input)) return;
            if(input=="CANCELAR" || input=="0"){ cout<<"Creación cancelada."<<endl; return; }
            if(input.empty()){ cout<<"El id no puede estar vacío."<<endl; continue; }
            try{ temp.id = stoi(input); }
            catch(...){ cout<<"ID inválido. Ingrese un número válido."<<endl; continue; }
            break;
        }
        ...
        if(resp=="S" || resp=="s" || resp=="Si" || resp=="SI" || resp=="si"){
            ...
            if(!archivo){ cout<<"Error al escribir proveedor en archivo binario."<<endl; return; }

            if(tienda->proveedores && tienda->cantidadProveedores < tienda->capacidadProveedores){
                tienda->proveedores[tienda->cantidadProveedores] = temp;
                tienda->cantidadProveedores++;
            }
            cout<<"Proveedor guardado en binario y memoria."<<endl;
        } else {
...
void editarProducto(Tienda* tienda, int idProducto){
    ...
    const char* ruta = "productos.bin";
    fstream archivo(ruta, ios::in | ios::out | ios::binary);
    if(!archivo){ cout<<"No se puede abrir el archivo de productos: "<<ruta<<"."<<endl; return; }

    int posArchivo = -1;
    Producto temp;
    Producto registro;
    int idx = 0;
    while(archivo.read(reinterpret_cast<char*>(&registro), sizeof(Producto))){
        if(registro.id == idProducto){
            posArchivo = idx;
            temp = registro;
            break;
        }
    ...
    if(posArchivo < 0){
        cout<<"Producto con ID "<<idProducto<<" no encontrado en el archivo binario."<<endl;
        archivo.close();
        return;
    }
    ...
    if(!encontrado){
        cout<<"Producto encontrado en archivo, pero no está cargado en memoria. Se editará solo binario."<<endl;
    }
        ...
        switch(ola){
            ...
            case 10:
                ...
                if(confirm=='S' || confirm=='s'){
                    archivo.clear();
                    archivo.seekp(posArchivo * sizeof(Producto), ios::beg);
                    archivo.write(reinterpret_cast<char*>(&temp), sizeof(Producto));
                    archivo.flush();
                    if(!archivo){
                        cout<<"Error al guardar cambios en el archivo binario."<<endl;
                    } else {
                        cout<<"Cambios guardados en binario."<<endl;
                    }

                    if(encontrado && idBuscado >= 0){
...
int main(){
            ...
            case 2:
                ...
                switch(opcion){
                    ...
                    case 2:
                        ...
                        {
                            Proveedor* encontrado = buscarProveedor(&tienda,id,nombre,opcion);
                            if(encontrado){
                                delete encontrado;
                            }
                        }
    ...
    if(!guardarTienda(&tienda)){
        cout<<"Advertencia: no se pudo guardar la tienda en tienda.bin"<<endl;
    }
