...
void Crearproductos(Tienda* tienda){
    ...
    for(int n=0;n<cantidad;n++){
        ...
        if(!getline(cin,input)) return; // Intenta leer la entrada del usuario, si falla (por ejemplo, si se encuentra un EOF o hay un error de entrada), la función retorna inmediatamente para evitar continuar con una entrada inválida.
        if(input=="CANCELAR" || input=="0"){ cout<<"Creación cancelada."<<endl; return; }
        ...
        string resp;
        if(!getline(cin,resp)) return; // Intenta leer la respuesta del usuario, si falla (EOF o error) sale de la función
...
void buscarProducto(Tienda* tienda){
    ...
    if(!getline(cin,input)) return;
    int idBuscado = 0;
    try{ idBuscado = stoi(input); } // Si la conversión falla, se lanzará una excepción que atrapamos para mostrar un mensaje de error y salir de la función
    catch(...){ cout<<"ID invalido."<<endl; return; } // Intentamos convertir la entrada a un número entero, si falla (por ejemplo, si el usuario ingresa texto no numérico), se captura la excepción y se muestra un mensaje de error indicando que el ID es inválido, luego se sale de la función para evitar continuar con una búsqueda con un ID no válido.
    for(int i=0;i<tienda->cantidadProductos;i++){ // Recorremos el arreglo de productos para buscar el ID ingresado por el usuario
        if(tienda->productos[i].id == idBuscado){ // Si encontramos un producto con el ID buscado, mostramos su información y salimos de la función
    ...
    string resp;
    if(!getline(cin,resp)) return; // Intenta leer toda la linea de la respuesta del usuario, si falla (EOF o error) sale de la función
    if (resp=="N" || resp=="n" || resp=="No" || resp=="NO" || resp=="no"){
        ...
        switch(resp){
            case "1":
                ...
                cin >> resp;
                    while (codigoDuplicado(tienda, resp)) { cout<<"Codigo ya existe. Coloque otro codigo."<<endl; cin >> resp; }
                    while (resp.empty()){ cout<<"El código no puede estar vacío. Edición cancelada."<<endl; cin >> resp; }
            ...
            case "2":
                ...
                if (tienda->productos[idBuscado-1].nombre != nullptr) delete[] tienda->productos[idBuscado-1].nombre; //Comprueba si el puntero no es nulo, y si no lo es libera la memoria asignada con el delete
                if (resp=="CANCELAR" || resp=="0"){ cout<<"Edición cancelada."<<endl; break; }
            ...
            cout<<"Ingrese la nueva descripción del producto: ";
                cin >> resp;
                while (resp.empty()){ cout<<"La descripción no puede estar vacía. Edición cancelada."<<endl; cin >> resp; }
                while (resp.length() >= sizeof(tienda->productos[idBuscado-1].descripcion )){ cout<<"La descripción es demasiado larga. Coloque otra descripción."<<endl; cin >> resp; }
                // Accede al producto encontrado usando idBuscado-1 (asumiendo que los IDs comienzan en 1 y corresponden a índices del arreglo) 
                //Y verifica si la longitud de la nueva descripción ingresada por el usuario es mayor o igual al tamaño máximo permitido para la descripción del producto. 
                //Si es así, muestra un mensaje indicando que la descripción es demasiado larga y solicita al usuario que ingrese otra descripción, 
                //repitiendo este proceso hasta que se ingrese una descripción válida.
                if (tienda->productos[idBuscado-1].descripcion != nullptr) delete[] tienda->productos[idBuscado-1].descripcion; //Comprueba si el puntero no es nulo, y si no lo es libera la memoria asignada con el delete
                if (resp=="CANCELAR" || resp=="0"){ cout<<"Edición cancelada."<<endl; break; }
                tienda->productos[idBuscado-1].descripcion = resp;
            ...
            case "4":
                ...
                if (resp=="CANCELAR" || resp=="0"){ cout<<"Edición cancelada."<<endl; break; }
                int idProv = 0;
                try{ idProv = stoi(resp); }
                catch(...){ cout<<"ID invalido. Edición cancelada."<<endl; break; }
                // Verifica si el ID del proveedor ingresado por el usuario existe en el arreglo de proveedores de la tienda. Si no existe, muestra un mensaje indicando que el proveedor no existe y cancela la edición.
                if (!existeProveedor(tienda, idProv)) { cout<<"Proveedor no existe. Edición cancelada."<<endl; break; }
                tienda->productos[idBuscado-1].idProveedor = idProv;
            ...
            case "5":
                ...
                cin >> resp;
                float nuevoPrecio = 0;
                while (nuevoPrecio <= 0){ cout<<"El precio debe ser mayor a 0. Introduzca un precio válido."<<endl; cin >> resp; // Solicita al usuario que ingrese un nuevo precio para el producto, y continúa solicitando hasta que se ingrese un precio válido (mayor a 0). Si el usuario ingresa un precio no válido, muestra un mensaje indicando que el precio debe ser mayor a 0 y solicita nuevamente.
                tienda->productos[idBuscado-1].precio = nuevoPrecio;
            ...
            case "6":
                ...
                cin >> resp;
                int nuevoStock = 0;
                while (nuevoStock <= 0){ cout<<"El stock debe ser mayor a 0. Introduzca un stock válido."<<endl; cin >> resp; } // Solicita al usuario que ingrese un nuevo stock para el producto, y continúa solicitando hasta que se ingrese un stock válido (mayor a 0). Si el usuario ingresa un stock no válido, muestra un mensaje indicando que el stock debe ser mayor a 0 y solicita nuevamente.
                tienda->productos[idBuscado-1].stock = nuevoStock; // Actualiza el stock del producto encontrado con el nuevo valor ingresado por el usuario.
